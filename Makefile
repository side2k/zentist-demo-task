...
last-reports:
	sqlite3 -header -line data/db.sqlite 'SELECT * FROM portal_runs WHERE id IN (SELECT MAX(id) FROM portal_runs GROUP BY portal_key) ORDER BY id DESC;'

last-errors:
	@for portal in orange_hrm saucedemo; do \
		last_id=$$(sqlite3 data/db.sqlite "SELECT MAX(id) FROM portal_runs WHERE portal_key='$$portal'"); \
		if [ -n "$$last_id" ]; then \
			echo "=== $$portal errors (run_id=$$last_id) ==="; \
			sqlite3 -header -column data/db.sqlite "SELECT input_item_id, error_message FROM $${portal}_errors WHERE run_id=$${last_id}"; \
		fi; \
	done
...
last-output:
	@for portal in orange_hrm saucedemo; do \
		...
		if [ -n "$$last_id" ]; then \
			...
			sqlite3 -header -column data/db.sqlite "SELECT $$cols FROM $${portal}_output WHERE run_id=$${last_id}"; \
		fi; \
	done
